...
async function login(req, res) {
    let data=req.body;
    try{
        let voter = await voterModel.findOne({aadharNumber:data.aadharNumber});
        if(voter && !voter.comparePassword(data.password)){
            ...
            res.status(200).json({token:generateToken(),message:'ok'});
        }
